...
pub async fn generate_tests(diff_info: &DiffInfo, _framework: &str, config: &Config) -> Result<String> {
    ...
    let system_prompt = "You are an expert software engineer writing simple, working unit tests. Generate ONLY basic test functions that will compile and run. Focus on testing simple functionality and configuration. Return ONLY the test code without any markdown formatting, explanations, or additional text.";
    
    let user_prompt = format!(
        "Generate simple unit tests for a {} project using {}. Focus ONLY on these working functions:\n\n1. Config::load() - test that config loads correctly\n2. Config::get_default_branch() - test default branch is 'master'\n3. Config::has_openai_key() - test API key detection\n4. call_openai_api() with no API key - test fallback behavior\n\nIMPORTANT:\n- Only test functions that actually exist\n- Use simple assertions like assert_eq! and assert!\n- Don't test complex AI functions that might fail\n- Make sure all imports are correct\n- Return only working test code\n\nCode changes:\n{}\n\nReturn only the raw test code, no explanations or markdown.",
...
fn create_rust_tests(_test_content: &str, test_dir: &Path) -> Result<()> {
    // Create simple, working tests without relying on AI generation
    let file_path = test_dir.join("simple_tests.rs");
    
    let content = r#"// Simple tests for commit-buddy
...
"#;
    ...
    println!("📝 Created test file: tests/simple_tests.rs");
    Ok(())
}
